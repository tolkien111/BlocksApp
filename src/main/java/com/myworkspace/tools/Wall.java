...
public class Wall implements Structure {
    ...
    public Optional<Block> findBlockByColor(String color) {
        ...
        return findByPredicate(x -> x.getColor().equals(colorToLowerCase))
                .findAny();
    }
    ...
    public List<Block> findBlocksByMaterial(String material) {
        if (material == null || material.isEmpty()
                || material.isBlank() || !material.matches("[a-zA-Z0-9]{3,}"))
            return new ArrayList<>();
        String materialToLowerCase = material.toLowerCase();
        return findByPredicate(y -> y.getMaterial().equals(materialToLowerCase))
                .toList();
    }
